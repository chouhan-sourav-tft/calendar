...
When(
  ...
  async (subject) => {
    ...
    await tickets.sendEmailFromCustomer(global.EMAIL, subject, newSubject);
  }
);
...
When('On ticket search page, search {string}', async (senderEmail) => {
  ...
  await baseAction.type(tickets.elements.filterSubject, newSubject);
});

Then('Validate that the ticket received is in {string}', async (option) => {
  await tickets.ValidateCloseTicket(option, newSubject);
});
