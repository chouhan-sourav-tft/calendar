...
exports.QualityManager = class QualityManager extends BaseAction {
  ...
  elements = {
    ...
    dateSortingDesc: '//table[@id="table_search_result_calls"]/thead/tr/th[8][@class="sorting_desc"]',
    dateSorting: '//table[@id="table_search_result_calls"]/thead/tr/th[8]',
  ...
  async searchCall(searchCall) {
    ...
    if(searchCall.audit){
      await this.selectOptionByValue(this.elements.auditSelect, searchCall.audit);
    }
    if (searchCall.phone) {
  ...
  async completeAuditsOfSelectedCalls(message) {
    ...
    if (message === 'error') {
      ...
      await this.click(this.elements.confirmButton);
      await this.isVisible(this.elements.completeAuditPopUp);
      // eslint-disable-next-line quotes
      await this.shouldContainText(
        this.elements.completeAuditPopUp,
        ' selected audits that are in the \'pending\' state?'
      );
    }
  }
  ...
  async validateCallTableDetails(tableData) {
    if(await this.isVisible(this.elements.dateSortingDesc)){
      const selectRow =
      ...
      await this.click(this.elements.dateSorting);
    }
  }
};
