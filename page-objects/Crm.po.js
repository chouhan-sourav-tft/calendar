...
exports.CRM = class Crm extends BaseAction {
  ...
  elements = {
    ...
    remoteTransferCallResult: '#remote-transfer-history-list div p:nth-child(2)',
    ...
    createdOutcomeGroup:'//li[contains(@class, "select2-results-dept-0")]/div[text()="Created outcomes"]',
    selectOutcomeGroup: '//li[contains(@class, "select2-results-dept-0")]/div[text()="System outcomes"]',
  ...
  async verifyRemoteTransferCall(datatable) {
    ...
    if(datatable.result) {
      await this.shouldContainText(this.elements.remoteTransferCallResult,datatable.result);
    }
  }
  ...
  async validateAssistedTransfer(callData) {
    await this.waitForSelector('#table-calls tr:nth-child('+callData.row+') .transfer-btn');
    await this.click('#table-calls tr:nth-child('+callData.row+') .transfer-btn');
  ...
  async changeContactOutcome(outcomeSettings) {
    ...
    if (outcomeSettings.changeTo === 'New') {
      ...
      if (outcomeSettings.outcomeGroup) {
        ...
        await this.click(`//li[contains(@class, "select2-results-dept-1")]/div[text()='${outcomeSettings.outcomeGroup}']`);
        await this.click(this.elements.selectOutcome);
        await this.click(`//li[contains(@class, "select2-results-dept-0")]/div[text()='${outcomeSettings.outcome}']`);
