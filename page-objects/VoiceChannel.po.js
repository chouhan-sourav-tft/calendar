...
exports.VoiceChannel = class VoiceChannel extends BaseAction {
  ...
  elements = {
    ...
    recycleTab: '[data-translate="recycle"]',
    ...
    callbackNextTime:'.xdsoft_timepicker.active div.xdsoft_time.xdsoft_current +div',
   ...
   */
  async transferCall() {
    await this.waitForSelector(this.elements.transferCallButton);
    await this.click(this.elements.transferCallButton);
  }
   ...
   * @return {void} Nothing
   */
  async selectTransferTab(transferType) {
   ...
   * @returns {void} nothing
   */
  async transferCallToQueue(queueName) {
    await this.scrollIntoElement(this.elements.ivrToggleButton);
    await this.click(this.elements.ivrToggleButton);
   ...
   * @return {void} Nothing
   */
  async muteQueueOriginalCall() {
    await this.waitForSelector(this.elements.muteCallQueueButton);
    await this.click(this.elements.muteCallQueueButton);
  }
   ...
   */
  async transferACall(transferType) {
    const locator = `//button[text() = '${transferType}']`;
    await this.click(locator);
    if (transferType === 'Assisted Transfer')
      await this.click(this.elements.transferAssistedCallButton);
  }
  ...
  async openCallbackManager() {
    ...
    await this.mouseOver(this.elements.callbackManager);
   ...
   * @return {void} Nothing
   */
  async createOutcome(outcome, attribute) {
    await this.waitForSelector(this.elements.outcomeNameInput);
