/*global page, secondSession, document, pageNew, promisify, context, window, thirdSession*/
...
exports.BaseAction = class BaseAction {
  ...
  async verifyelementIsHidden(locator, visibility, type = '') {
    if (type === 'second') {
      let visible = await global.secondSession.locator(locator).isHidden({ timeout: defaultTimeout });
      await expect(visible).to.equal(visibility);
    }
    if (type === 'third') {
      let visible = await global.thirdSession.locator(locator).isHidden({ timeout: defaultTimeout });
      await expect(visible).to.equal(visibility);
    }else{
  ...
  async waitForSelector(selector, pageType = '', timeOut = defaultTimeout) {
    if (pageType === 'newTab') {
      await global.pageNew.waitForSelector(selector, {state: 'visible', timeout: timeOut });
    } else if (pageType === 'second') {
