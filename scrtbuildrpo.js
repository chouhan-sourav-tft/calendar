...
exports.ScriptBuilder = class ScriptBuilder extends BaseAction {
  ...
  elements = {
    ...
    scriptValuesRequired: '//div[@class="form-group required_class"]//input[@id="item_required"]',
    scriptValuesRequiredAlways: '//div[@class="form-group required_class always_required_class"]//input[@id="item_required_always"]',
    minimalAnswerInput: '#item_req_minimal-answers-input',
    callback: '[class="item form-group callback_class sortable ui-draggable"]>label',
    callbackSelect : ' #script-din-left-ctn [data-translate="callback.schedule.label"]',
    campaignDropDown: '#s2id_select_callback_campaign_queue_to_another_agent',
    campaignSearch: '[class="select2-drop select2-display-none select2-with-searchbox select2-drop-active"] input',
    ...
    calendarElement: 'div[title="Calendars"]',
  ...
  async fillNewScriptDetails(scriptName, newScriptForm) {
    ...
    if (newScriptForm.ticketQueueName) {
      await this.waitForSelector(this.elements.newScriptModalElements.ticketQueues);
      await this.type(
    ...
    await this.forceClick(this.elements.newScriptModalElements.createButton);
  }
  ...
  async saveRule(formFields = {}) {
    ...
    if (formFields.queue) {
      await this.waitForSelector(this.elements.generalRulesModalElements.ticketQueues);
   ...
   * @param {object} itemConfigurations - item configurations object
   * @param {string} itemConfigurations.calendartitle - element title
   * @param {string} itemConfigurations.calendarName - element name
  ...
  async calendarItems(itemConfigurations) {
    ...
    await this.pressKey('Enter');
    await this.click(this.elements.requiredBooking);
    await this.dropdownOptionSelect(this.elements.requiredBooking,itemConfigurations.requiredBooking);
    await this.click(this.elements.saveEdits);
    await this.waitForSelector(this.elements.successMessage);
  }
  ...
  async setCallback(callbackDetails){
    if(callbackDetails.callbackType){
      await this.waitForSelector(`//span[text()="${callbackDetails.callbackType}"]`);
    ...
    if(callbackDetails.campaign){
      ...
      await this.click(`//div[@class="select2-result-label" and text()="${callbackDetails.campaign}"]`);
    }
    if(callbackDetails.relatedDB){
      ...
      await this.click(`//div[@class="select2-result-label" and text()="${callbackDetails.relatedDB}"]`);
    }
    if(callbackDetails.agent){
      ...
      await this.click(`//div[@class="select2-result-label" and text()="${callbackDetails.agent}"]`);
  ...
  async bookEvent(hour, desc, days) {
    ...
    await this.type(this.elements.eventDescription, desc);
